...
| 3b | `core/commands.ts` is listed in the plan's file structure but no task creates it. Without it there is no root `Command` union and no root `decide`. | **RESOLVED — Task 21 created `core/decide.ts`** because it needed a dispatcher to wire its property commands. Task 19 must EXTEND that file rather than create it, adding the Settlement command and any remaining contexts. Verify its dispatch covers every context with a decider before Task 19 builds on it. |
